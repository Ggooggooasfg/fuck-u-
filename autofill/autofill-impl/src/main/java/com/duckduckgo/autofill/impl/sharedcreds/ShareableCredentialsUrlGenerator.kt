...
class RealShareableCredentialsUrlGenerator @Inject constructor(
    ...
    private fun unidirectionalShareableUrls(
        ...
        config.unidirectionalRules.forEach { rule ->
            ...
            val matches = rule.from.filter { autofillUrlMatcher.matchingForAutofill(visitedSiteUrlParts, it) }
            matches.forEach { _ ->
                unidirectionalMatches.addAll(rule.to.minus(visitedSiteUrlParts))
            }
        }
    ...
    private fun omnidirectionalShareableUrls(
        ...
        config.omnidirectionalRules.forEach { rule ->
            ...
            matches.forEach { _ ->
                omnidirectionalMatches.addAll(rule.shared.minus(visitedSiteUrlParts))
            }
        }
