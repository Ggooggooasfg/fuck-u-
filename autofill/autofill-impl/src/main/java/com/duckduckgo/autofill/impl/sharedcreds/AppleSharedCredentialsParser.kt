...
class AppleSharedCredentialsParser @Inject constructor(
    ...
    private fun convertJsonToRules(json: String?): SharedCredentialConfig {
        ...
        rules.forEach { rule ->
            if (rule.shared != null) {
                val sharedList = rule.shared.map { autofillUrlMatcher.extractUrlPartsForAutofill(it) }
                omnidirectionalRules.add(OmnidirectionalRule(sharedList))
            } else if (rule.from != null && rule.to != null) {
                val fromList = rule.from.map { autofillUrlMatcher.extractUrlPartsForAutofill(it) }
                val toList = rule.to.map { autofillUrlMatcher.extractUrlPartsForAutofill(it) }
